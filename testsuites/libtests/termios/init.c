...
void print_c_iflag( struct termios *tp )
{
  ...
  print_32bits(
    tp->c_iflag,
    sizeof( c_iflag_bits ) / sizeof( char * ),
    c_iflag_bits
  );
  printf( "\n" );
}
...
void print_c_lflag( struct termios *tp )
{
  ...
  print_32bits(
    tp->c_lflag,
    sizeof( c_lflag_bits ) / sizeof( char * ),
    c_lflag_bits
  );
  printf( "\n" );
}
...
unsigned long get_baud_rate( void )
{
  ...
  while ( 1 ) {
    ...
    printf(
      "Choices are: 50, 75, 110, 134, 150, 200, 300, 600, 1200, 1800\n"
    );
    printf(
      "2400, 4800, 9600, 19200, 38400, 7200, 14400, 28800, 57600, 76800\n"
    );
...
unsigned long get_parity( void )
{
  ...
  while ( 1 ) {
    ...
    printf(
      "Choices are: 0 for no parity, 1 for even parity, 2 for odd parity\n"
    );
    printf( "\nYour choice: " );
    scanf( "%d", &parity );
    printf( "\n" );
...
void change_line_settings( struct termios *tp )
{
  ...
  printf(
    "NOTE: You will not see output until you switch your terminal settings!\n"
  );
  printf(
    "WARNING: If you do not switch your terminal settings, your terminal may hang.\n"
  );
  printf(
    "Enter the number of seconds the test will wait for you to switch your terminal\n"
  );
  ...
  printf( "\n" );
  printf(
    "Setting line to new termios settings in %lu seconds.\n",
    sleep_time
  );
...
void canonical_input( struct termios *tp )
{
  ...
  tp->c_lflag = ISIG | ICANON | ECHO | ECHONL | ECHOK | ECHOE | ECHOPRT |
                ECHOCTL | IEXTEN;
...
void do_raw_input( int vmin, int vtime )
{
  ...
  do {
    ...
    printf(
      "Count:%-10lu  Interval:%3u.%3.3d  Char:",
      count,
      msec / 1000,
      msec % 1000
    );
