...
#define ATOMIC_FETCH_AND_NO_BARRIER(TYPE, cpuid)         \
{                                                        \
  ...
  for (i = 0; i < TEST_REPEAT; i++){                     \
    ...
    t = a;                                               \
    _Atomic_Fetch_and_##TYPE(&t, b, ATOMIC_RELAXED_BARRIER);  \
    ...
    )
{
  ...
  ATOMIC_FETCH_AND_NO_BARRIER(int, cpu_num);

  ATOMIC_FETCH_AND_NO_BARRIER(long, cpu_num);

  ATOMIC_FETCH_AND_NO_BARRIER(ptr, cpu_num);

  ATOMIC_FETCH_AND_NO_BARRIER(32, cpu_num);
