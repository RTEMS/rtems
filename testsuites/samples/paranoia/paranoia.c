...
#define basename( cp ) \
  ( ( temp = (char *) strrchr( ( cp ), '/' ) ) ? temp + 1 : ( cp ) )
...
int main( int argc, char **argv )
{
  ...
  TstCond(
    Failure,
    ( Three == Two + One ) && ( Four == Three + One ) &&
      ( Four + Two * ( -Two ) == Zero ) && ( Four - Three - One == Zero ),
    "3 != 2+1, 4 != 3+1, 4+2*(-2) != 0, or 4-3-1 != 0"
  );
  TstCond(
    Failure,
    ( MinusOne == ( 0 - One ) ) && ( MinusOne + One == Zero ) &&
  ...
  TstCond(
    Failure,
    ( Nine == Three * Three ) && ( TwentySeven == Nine * Three ) &&
  ...
  do {
    do {
      Y1 = Y;
      Y = ( Half -
            ( ( Half - ( Half * Y1 + ThirtyTwo * Y1 * Y1 ) ) + Half ) ) +
          Half;
    } while ( !( ( Y1 <= Y ) || ( Y <= Zero ) ) );
    X1 = X;
...
or  1/3  and  3/9  and  9/27 may disagree"
  ...
  if ( ( X == Zero ) && ( Y == Zero ) && ( Z == Zero ) && ( T <= Zero ) ) {
    ...
    Y1 = Y1 - Half;
    if (
      ( X == Zero ) && ( Y == Zero ) && ( Z == Zero ) && ( T == Zero ) &&
      ( StickyBit == Zero ) && ( Y1 == Half )
    ) {
      RMult = Rounded;
      printf( "Multiplication appears to round correctly.\n" );
    } else if (
      ( X + U2 == Zero ) && ( Y < Zero ) && ( Z + U2 == Zero ) &&
  ...
  StickyBit = Zero;
  if (
    ( GMult == Yes ) && ( GDiv == Yes ) && ( GAddSub == Yes ) &&
    ( RMult == Rounded ) && ( RDiv == Rounded ) && ( RAddSub == Rounded ) &&
...
(noted above) count as one flaw in the final tally below"
  ...
  if ( I == NoTrials ) {
    ...
    Z = Random1 * Random2;
    Y = Random2 * Random1;
    Z9 = ( One + Half / Three ) * ( ( U2 + U1 ) + One ) -
         ( One + Half / Three ) * ( ( U2 + U1 ) + One );
  }
  ...
  if ( !Anomaly ) {
    if ( !( ( MinSqEr < Zero ) || ( MaxSqEr > Zero ) ) ) {
      ...
      if (
        ( MaxSqEr + U2 > U2 - Half ) || ( MinSqEr > Half ) ||
        ( MinSqEr + Radix < Half )
      ) {
        SqRWrng = True;
      } else {
  ...
  if ( D <= C ) {
    ...
    do {
      ...
      ( Underflow > PseudoZero ) && ( PseudoZero + PseudoZero > PseudoZero )
    );
  }
  ...
  switch ( I ) {
    ...
    case 4:
      if (
        ( Q == UfThold ) && ( E1 == E0 ) && ( FABS( UfThold - E1 / E9 ) <= E1 )
      ) {
        UfNGrad = False;
...
the system traps on overflow.\n" );
  ...
  for ( Indx = 1; Indx <= 3; ++Indx ) {
    ...
    if ( Z != Zero ) {
      ...
      Y = V9 * V9;
      if (
        Y / ( One - Radix * E9 ) < Z || Y > ( One + Radix * E9 ) * Z
      ) { /* dgh: + E9 --> * E9 */
        if ( V9 > U1 ) {
          BadCond( Serious, "" );
        } else {
  ...
  printf( "\n" );
  if (
    ( ErrCnt[ Failure ] + ErrCnt[ Serious ] + ErrCnt[ Defect ] +
      ErrCnt[ Flaw ] ) > 0
  ) {
    if (
      ( ErrCnt[ Failure ] + ErrCnt[ Serious ] + ErrCnt[ Defect ] == 0 ) &&
      ( ErrCnt[ Flaw ] > 0 )
    ) {
    ...
    if ( !( ( RMult == Rounded ) && ( RDiv == Rounded ) &&
      ...
      if (
        StickyBit >= One && ( Radix - Two ) * ( Radix - Nine - One ) == Zero
      ) {
        ...
        if (
          ...
          ( ( Precision - Four * Three * Two ) *
              ( Precision - TwentySeven - TwentySeven + One ) ==
            Zero )
        ) {
          printf( "754" );
        } else {
