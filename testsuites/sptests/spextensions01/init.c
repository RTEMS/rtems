...
{
  assert(
    _Thread_Dispatch_is_enabled() || before_initialization() ||
    before_multitasking()
  );
  assert( _RTEMS_Allocator_is_owner() );
  assert( life_protected() );
...
{
  assert(
    ( counter % active_extensions ) == ( active_extensions - 1 - index )
  );
  ++counter;
}
...
static const rtems_extensions_table
  initial_test = RTEMS_TEST_INITIAL_EXTENSION;

#ifdef BSP_INITIAL_EXTENSION
static const rtems_extensions_table initial_bsp = BSP_INITIAL_EXTENSION;
#endif
