...
Name:         	rtems-4.7-avr-rtems4.7-gcc
Summary:      	avr-rtems4.7 gcc
...
BuildRequires:	rtems-4.7-avr-rtems4.7-binutils

Requires:	rtems-4.7-gcc-common
Requires:	rtems-4.7-avr-rtems4.7-binutils
Requires:	rtems-4.7-avr-rtems4.7-newlib = %{newlib_version}-%{release}
...
# Misplaced header file
  if test -f $RPM_BUILD_ROOT%{_includedir}/mf-runtime.h; then
    mv $RPM_BUILD_ROOT%{_includedir}/mf-runtime.h \
      $RPM_BUILD_ROOT%{gcclib}/avr-rtems4.7/%{gcc_version}/include/
  fi
%endif
  ...
  rm -f $RPM_BUILD_ROOT%{_bindir}/avr-rtems4.7-c++filt%{_exeext}
...
%if "%{gcc_version}" >= "3.4"
  # Bug in gcc-3.4.0pre
  rm -f $RPM_BUILD_ROOT%{_bindir}/avr-rtems4.7-avr-rtems4.7-gcjh%{_exeext}
%endif

%if "%{gcc_version}" >= "3.3"
  # Bug in gcc-3.3.x/gcc-3.4.x: Despite we don't need fixincludes, it installs
  # the fixinclude-install-tools
  rm -rf ${RPM_BUILD_ROOT}%{gcclib}/avr-rtems4.7/%{gcc_version}/install-tools
  rm -rf ${RPM_BUILD_ROOT}%{gccexec}/avr-rtems4.7/%{gcc_version}/install-tools
%endif
  ...
  TGTDIR="%{_exec_prefix}/avr-rtems4.7/lib"
  for i in $f; do
    case $i in
...
%endif
  ...
  echo "%dir %{gcclib}/avr-rtems4.7" >> dirs

  TGTDIR="%{gcclib}/avr-rtems4.7/%{gcc_version}"
  for i in $f; do
    case $i in
...
#!/bin/sh
grep -E -v '^${RPM_BUILD_ROOT}%{_exec_prefix}/avr-rtems4.7/(lib|include|sys-root)' \
  | grep -v '^${RPM_BUILD_ROOT}%{gcclib}/avr-rtems4.7/' | %__find_provides
EOF
...
#!/bin/sh
grep -E -v '^${RPM_BUILD_ROOT}%{_exec_prefix}/avr-rtems4.7/(lib|include|sys-root)' \
  | grep -v '^${RPM_BUILD_ROOT}%{gcclib}/avr-rtems4.7/' | %__find_requires
EOF
...
# ==============================================================
# rtems-4.7-avr-rtems4.7-gcc
# ==============================================================
# %package -n rtems-4.7-avr-rtems4.7-gcc
# Summary:        GNU cc compiler for avr-rtems4.7
# Group:          Development/Tools
# Version:        %{gcc_rpmvers}
# Requires:       rtems-4.7-avr-rtems4.7-binutils
# Requires:       rtems-4.7-avr-rtems4.7-newlib = %{newlib_version}-%{release}
...
%description -n rtems-4.7-avr-rtems4.7-gcc
GNU cc compiler for avr-rtems4.7.

%files -n rtems-4.7-avr-rtems4.7-gcc -f build/files.gcc
%defattr(-,root,root)
%dir %{_mandir}
%dir %{_mandir}/man1
%{_mandir}/man1/avr-rtems4.7-gcc.1*
%if "%{gcc_version}" >= "3.4"
%{_mandir}/man1/avr-rtems4.7-cpp.1*
%{_mandir}/man1/avr-rtems4.7-gcov.1*
%endif

%dir %{_bindir}
%{_bindir}/avr-rtems4.7-cpp%{_exeext}
%{_bindir}/avr-rtems4.7-gcc%{_exeext}
%if "%{gcc_version}" >= "3.3"
%{_bindir}/avr-rtems4.7-gcc-%{gcc_version}%{_exeext}
%endif
%{_bindir}/avr-rtems4.7-gcov%{_exeext}
%{_bindir}/avr-rtems4.7-gccbug

%dir %{gcclib}/avr-rtems4.7/%{gcc_version}/include
%if "%{gcc_version}" > "4.0.3"
%if "avr-rtems4.7" != "bfin-rtems4.7"
%dir %{gcclib}/avr-rtems4.7/%{gcc_version}/include/ssp
%endif
%endif

%dir %{gccexec}
%dir %{gccexec}/avr-rtems4.7
%dir %{gccexec}/avr-rtems4.7/%{gcc_version}
%{gccexec}/avr-rtems4.7/%{gcc_version}/cc1%{_exeext}
%{gccexec}/avr-rtems4.7/%{gcc_version}/collect2%{_exeext}
...
# ==============================================================
# rtems-4.7-avr-rtems4.7-newlib
# ==============================================================
%package -n rtems-4.7-avr-rtems4.7-newlib
Summary:      	C Library (newlib) for avr-rtems4.7
Group: 		Development/Tools
License:	Distributable
Version:	%{newlib_version}

Provides:	rtems-4.7-avr-rtems4.7-libc = %{newlib_version}-%{release}
Obsoletes:	rtems-4.7-avr-rtems4.7-libc < %{newlib_version}-%{release}

Requires:	rtems-4.7-newlib-common

%description -n rtems-4.7-avr-rtems4.7-newlib
Newlib C Library for avr-rtems4.7.

%files -n rtems-4.7-avr-rtems4.7-newlib -f build/files.newlib
%defattr(-,root,root)
%dir %{_prefix}
%dir %{_exec_prefix}/avr-rtems4.7
%{_exec_prefix}/avr-rtems4.7/include
