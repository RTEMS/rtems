...
%build
  ...
  mkdir -p amd64-pc-netbsdelf5.0.2/sys-root
  %{__tar} cf - lib usr/lib usr/include | ( cd amd64-pc-netbsdelf5.0.2/sys-root ; %{__tar} xf -)

  pushd amd64-pc-netbsdelf5.0.2/sys-root/usr/lib > /dev/null
  # missing files
  rm -f lib*_p.*
  # not needed
  rm -rf aout compat
...
%install
  ...
  cp -a amd64-pc-netbsdelf5.0.2 $RPM_BUILD_ROOT%{_prefix}
...
%package sys-root
Group: Development/Tools
Summary: amd64-pc-netbsdelf5.0.2 target files for gcc >= 3.4

%description sys-root
amd64-pc-netbsdelf5.0.2 target files for gcc >= 3.4

%files sys-root
%defattr(-,root,root,-)
%dir %{_prefix}/amd64-pc-netbsdelf5.0.2
%{_prefix}/amd64-pc-netbsdelf5.0.2/sys-root
