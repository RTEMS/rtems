 ...
 */
typedef uint8_t rtems_memory_management_permission;

/**
 * rtems_memory_management_permission bit masks
 */
#define RTEMS_MEMORY_MANAGEMENT_NO_PERMISSION           0x00
#define RTEMS_MEMORY_MANAGEMENT_READ_PERMISSION         0x01
#define RTEMS_MEMORY_MANAGEMENT_WRITE_PERMISSION        0x02
#define RTEMS_MEMORY_MANAGEMENT_EXECUTE_PERMISSION      0x04
#define RTEMS_MEMORY_MANAGEMENT_CACHEABLE_PERMISSION    0x08
// TODO: SHAREABLE and GUARDED
#define RTEMS_MEMORY_MANAGEMENT_ALL_PERMISSION          0x0f

/**
 *  @brief rtems_memory_management_has_read_permission
 *
 *  Returns non-zero if @a _mpe has read permission bit set.
 */
#define rtems_memory_management_has_read_permission(_mpe) \
  (_mpe->permissions & RTEMS_MEMORY_MANAGEMENT_READ_PERMISSION)

/**!
 *  @brief rtems_memory_management_has_write_permission
 *
 *  Returns non-zero if @a _mpe has write permission bit set.
 */
#define rtems_memory_management_has_write_permission(_mpe) \
  (_mpe->permissions & RTEMS_MEMORY_MANAGEMENT_WRITE_PERMISSION)

/**
 *  @brief rtems_memory_management_has_execute_permission
 *
 *  Returns non-zero if @a _mpe has execute permission bit set.
 */
#define rtems_memory_management_has_execute_permission(_mpe) \
  (_mpe->permissions & RTEMS_MEMORY_MANAGEMENT_EXECUTE_PERMISSION)

/**
 *  @brief rtems_memory_management_has_cacheable_permission
 *
 *  Returns non-zero if @a _mpe has cacheable permission bit set.
 */
#define rtems_memory_management_has_cacheable_permission(_mpe) \
  (_mpe->permissions & RTEMS_MEMORY_MANAGEMENT_CACHEABLE_PERMISSION)
 ...
 * by a rtems_memory_management_region_descriptor with a set of permissions.
 * If it is currently being enforced then the installed field is true.
 */
typedef struct
{
  rtems_chain_node                          node;
  rtems_memory_management_region_descriptor region;     /* protected region */
  bool                                      installed;  /* true if protected */
  rtems_memory_management_permission        permissions;
...
typedef struct
{
  rtems_memory_management_entry  *mpe_array;
  size_t                          mpe_array_size;
  rtems_chain_control             active_mpe_chain;
  rtems_chain_control             idle_mpe_chain;
} rtems_memory_management_domain;

/**
 * @brief rtems_memory_management_initialize
 *
 * Calls _CPU_Memory_management_Initialize to initialize the hardware
 * for memory protection. Implemented in libcpu/shared/
 */
rtems_status_code rtems_memory_management_initialize( void );

/**
 * @brief rtems_memory_management_verify_permission
 *
 * Calls _CPU_Memory_management_Verify_permission to verify the hardware
...
/**
 * @brief rtems_memory_management_verify_size
 *
 * Calls _CPU_Memory_management_Verify_size to verify the hardware
...
/**
 * @brief rtems_memory_management_uninstall_entry
 *
 * Calls _CPU_Memory_management_Uninstall_MPE to remove the @a mpe from
...
/**
 *  @brief rtems_memory_management_initialize_domain
 *
...
rtems_status_code rtems_memory_management_initialize_domain(
  rtems_memory_management_domain *domain,
  size_t max_number_of_mpes
);

/**
 *  @brief rtems_memory_management_finalize_domain
 *
...
/**
 * @brief rtems_memory_management_install_domain
 *
...
/**
 * @brief rtems_memory_management_uninstall_domain
 *
...
/**
 * @brief rtems_memory_management_create_entry
 *
...
rtems_status_code rtems_memory_management_create_entry(
  ...
  rtems_memory_management_entry** mpe_ret
);

/**
 * @brief rtems_memory_management_delete_entry
 *
...
/**
 * @brief rtems_memory_management_find_entry
 *
...
/**
 * @brief rtems_memory_management_set_permission
 *
...
(
  rtems_memory_management_entry* const mpe,
  const rtems_memory_management_permission new_perms,
  rtems_memory_management_permission*  old_perms
);
...
rtems_status_code rtems_memory_management_set_read
(
  rtems_memory_management_entry* const mpe;
);

rtems_status_code rtems_memory_management_set_execute
(
  rtems_memory_management_entry* const mpe;
);
