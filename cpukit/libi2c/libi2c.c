 ...
 *
 * adaptations to use RTEMS API for mutex handling
 * by Vijay Banerjee
 */
#ifdef HAVE_CONFIG_H
#include "config.h"
...
#define MAX_NO_DRIVERS \
  16 /* Number of high level drivers we support          */
...
#define DECL_CHECKED_DRV( d, b, m )                                      \
  ...
  unsigned b = MINOR2BUS( m );                                           \
  if (                                                                   \
    b >= MAX_NO_BUSSES || 0 == busses[ b ].bush || d > MAX_NO_DRIVERS || \
    ( d && 0 == drvs[ d - 1 ].drv )                                      \
  ) {                                                                    \
...
static rtems_mutex libmutex = RTEMS_MUTEX_INITIALIZER("LIBI2C");
...
static void lock_bus( int busno )
{
  rtems_recursive_mutex_lock(&busses[busno].mutex);
}

static void unlock_bus( int busno )
{
  rtems_recursive_mutex_unlock(&busses[busno].mutex);
}

/* Note that 'arg' is always passed in as NULL */
rtems_status_code rtems_i2c_init(
...
rtems_status_code rtems_i2c_open(
  rtems_device_major_number major,
  rtems_device_minor_number minor,
  void                     *arg
)
...
rtems_status_code rtems_i2c_close(
  rtems_device_major_number major,
  rtems_device_minor_number minor,
  void                     *arg
)
...
rtems_status_code rtems_i2c_read(
  rtems_device_major_number major,
  rtems_device_minor_number minor,
  void                     *arg
)
{
  ...
  if ( 0 == drv ) {
    rval = rtems_libi2c_start_read_bytes(
      minor,
...
rtems_status_code rtems_i2c_write(
  rtems_device_major_number major,
  rtems_device_minor_number minor,
  void                     *arg
)
{
  ...
  if ( 0 == drv ) {
    rval = rtems_libi2c_start_write_bytes(
      minor,
      (unsigned char *) rwargs->buffer,
      rwargs->count
    );
...
rtems_status_code rtems_i2c_ioctl(
  rtems_device_major_number major,
  rtems_device_minor_number minor,
  void                     *arg
)
...
int rtems_libi2c_register_bus( const char *name, rtems_libi2c_bus_t *bus )
{
  ...
  if ( '/' != *nmcpy ) {
    safe_printf(
      DRVNM "Bad name: must be an absolute path starting with '/'\n"
    );
    free( nmcpy );
  ...
  *chpt = tmp;
  ...
  for (i = 0; i < MAX_NO_BUSSES; i++) {
    ...
    if (!busses[i].bush) {
      /* found a free slot */
      char bus_mtx[5];
      snprintf(bus_mtx, 5, "I2C%d", i);
      rtems_recursive_mutex_init(&busses[i].mutex, bus_mtx);
...
static int not_started( int busno )
{
  return !busses[ busno ].started;
}
...
rtems_status_code rtems_libi2c_send_addr(
  rtems_device_minor_number minor,
  int                       rw
)
{
...
int rtems_libi2c_start_read_bytes(
  rtems_device_minor_number minor,
  unsigned char            *bytes,
  int                       nbytes
)
...
int rtems_libi2c_start_write_bytes(
  rtems_device_minor_number minor,
  const unsigned char      *bytes,
  int                       nbytes
)
...
)
{
  ...
  if (
    busno >= MAX_NO_BUSSES || !busses[ busno ].bush || i2caddr >= 1 << 10
  ) {
    errno = EINVAL;
    return -RTEMS_INVALID_NUMBER;
  }
  ...
  if ( drvtbl == NULL || drvtbl->size < sizeof( *drvtbl ) ) {
    safe_printf(
      DRVNM "No driver table or size too small -- misconfiguration?\n"
    );
    return -RTEMS_NOT_CONFIGURED;
  }
  ...
  for (i = 0; i < MAX_NO_DRIVERS; i++) {
    ...
    if ( !drvs[ i ].drv ) {
      ...
      if ( drvtbl->ops->initialization_entry ) {
        err = drvs[ i ].drv->ops->initialization_entry(
          rtems_libi2c_major,
          minor,
          0
        );
