...
static rtems_disk_device *get_disk_entry( dev_t dev, bool lookup_only )
{
...
static rtems_disk_device **create_disk_table_entry( dev_t dev )
{
  rtems_device_major_number major = 0;
  rtems_device_minor_number minor = 0;
...
static rtems_status_code create_disk(
  dev_t               dev,
  const char         *name,
...
static int null_handler( rtems_disk_device *dd, uint32_t req, void *argp )
{
  (void) dd;
  (void) req;
  (void) argp;
...
static bool is_physical_disk( const rtems_disk_device *dd )
{
...
)
{
  ...
  sc = rtems_disk_init_log( dd, phys_dd, block_begin, block_count );
...
static void free_disk_device( rtems_disk_device *dd )
{
...
static void rtems_disk_cleanup( rtems_disk_device *disk_to_remove )
{
