 ...
 *
 * @brief This source file contains the ARM relocation support of the
 *   dynamic linker.
 */
...
static inline Elf_Addr load_ptr(void* where) {
  Elf_Addr res;
...
static void* set_veneer(void* trampoline, Elf_Addr target) {
  /*
...
static void* set_thumb_bx_veneer(void* trampoline, Elf_Addr target) {
  /*
   * http://shell-storm.org/online/Online-Assembler-and-Disassembler/
   *
   *  Thumb mode:
...
static size_t get_veneer_size(int type) {
  (void)type;
...
rtems_rtl_elf_reloc_rel(rtems_rtl_obj* obj, const Elf_Rel* rel,
                        const rtems_rtl_obj_sect* sect, const char* symname,
                        const Elf_Byte syminfo, const Elf_Word symvalue,
                        const bool parsing) {
  (void)symname;
  ...
  case R_TYPE(JUMP24): /* B/BL<cond> */
    ...
    if (((Elf_Sword)tmp > 0x7fffff) || ((Elf_Sword)tmp < -0x800000)) {
      ...
      if (!rtems_rtl_obj_has_tramp_space(obj, tramp_size)) {
        rtems_rtl_set_error(
            EINVAL, "%s: CALL/JUMP24: overflow: no tramp memory", sect->name);
        return rtems_rtl_elf_rel_failure;
      }
    ...
    if (!parsing) {
      ...
      *(uint16_t*)where =
          (uint16_t)((upper_insn & 0xfbf0) | ((tmp & 0xf000) >> 12) |
                     ((tmp & 0x0800) >> 1));
      *((uint16_t*)where + 1) =
          (uint16_t)((lower_insn & 0x8f00) | ((tmp & 0x0700) << 4) |
                     (tmp & 0x00ff));
  ...
  case R_TYPE(THM_PC22):
    if (parsing) {
      ...
      tmp = (i1 << 23) | (i2 << 22) | ((upper_insn & 0x3ff) << 12) |
            ((lower_insn & 0x7ff) << 1);
    ...
    if (isThumb(symvalue) == false &&
      ...
      if (!rtems_rtl_obj_has_tramp_space(obj, tramp_size)) {
        rtems_rtl_set_error(EINVAL, "%s: THM_JUMP24: overflow: no tramp memory",
                            sect->name);
        return rtems_rtl_elf_rel_failure;
      }
  ...
  case R_TYPE(THM_JUMP19):
    ...
    if ((((upper_insn & 0x3f) >> 7) & 0x7) == 0x7) {
      ...
      return false; /*if cond <3:1> == '111', see Related codings in armv7a
                       manual */
    }
    ...
    tmp = ((i2 << 19) | (i1 << 18) | ((upper_insn & 0x3f) << 12) |
           ((lower_insn & 0x7ff) << 1));
    ...
    *(uint16_t*)where =
        (upper_insn & 0xfbc0) | (sign << 10) | ((tmp >> 12) & 0x3f);
    *((uint16_t*)where + 1) =
...
rtems_rtl_elf_rel_status rtems_rtl_elf_relocate_rel_tramp(
    ...
    const char* symname, const Elf_Byte syminfo, const Elf_Word symvalue) {
  return rtems_rtl_elf_reloc_rel(obj, rel, sect, symname, syminfo, symvalue,
                                 true);
}
